...
class ByteCodeGenerator extends TestsGenerator {
    ...
    private void generateClassFiles(IRNode mainClass, IRNode privateClasses) {
        ...
        try {
            ...
            try (PrintWriter pw = new PrintWriter(Files.newOutputStream(errFile,
                ...
                t.printStackTrace();
                throw generationFailure("can't write error to error file " + errFile, e);
            }
            throw generationFailure("Can't generate bytecode test: " + mainClassName, t);
        }
    }
    ...
    private void writeFile(String fileName, byte[] bytecode) {
        try {
            ...
            throw generationFailure("Can't write generated file: " + generatorDir.resolve(fileName), ex);
        }
    }
