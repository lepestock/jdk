...
#            echo "Outcome: Generation failure"
    ...
    if [ $success -eq 0 ]; then
#        echo ":gentest-build"
        repl-gentest-build > /dev/null 2>&1; success=$?
        [ $success != 0 ] && echo "Outcome: Compilation error"
    fi
...
#        echo ":gentest-run";
        ...
        success=$?
        if [ $success -eq "124" ]; then
            echo "Outcome: run timeout"
#        elif [ $success -ne "0"]; then         # Maybe we'll need to separate those in the future....
#            echo "Outcome: Early stop"
        elif [ $(cat testbase/Gentest.output | wc -l) -lt "30" ]; then
            echo "Outcome: Early stop"
        fi
    fi
}
